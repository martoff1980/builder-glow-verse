...
const SUGGESTED_RUMORS = {
  ...
  sector: [
    ...
    {
      text: "Фінсектор посилює капіталізацію, кредити відновлюються",
      cred: 0.7,
    },
...
export default function TradingSimulator({
  ...
  async function createRumorServer() {
    try {
      ...
      game.createRumor(
        data.content,
        data.credibility,
        (data.target || undefined) as string | undefined,
      );
      setTick((t) => t + 1);
  ...
  const price = useMemo(
    () => game.market.getCurrentPrice(selected),
    [game, selected, tick],
  );
  ...
  const totalHoldingsValue = useMemo(
    () => game.player.getPortfolioValue(game.market),
    [game, tick],
  );
  ...
  const history = (game.market.history.get(selected) || []).map((p) => ({
    t: p.day,
    price: p.price,
  }));
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <Card className="lg:col-span-2">
          <CardHeader>
            ...
            <CardDescription>
              День {game.timeSystem.currentDay} • Репутація{" "}
              {Math.round(game.player.reputation)} • Розслідування{" "}
              {Math.round(game.legalSystem.investigationLevel)}%
              {game.player.jailTime > 0
                ? ` • В'язниця ${game.player.jailTime}д`
                : ""}
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="flex items-end gap-2">
                <Button
                  className="flex-1"
                  onClick={onBuy}
                  disabled={game.player.jailTime > 0}
                >
                  Купити
                </Button>
                <Button
            ...
            <div className="flex flex-wrap items-center gap-2">
                  ...
                  <div className="mt-3 grid grid-cols-1 md:grid-cols-4 gap-3">
                    <div className="md:col-span-3">
                      <p className="text-sm text-muted-foreground mb-2">
                        Популярні біржові чутки
                      </p>
                      <div className="border-2 border-blue-500 grid grid-cols-1 sm:grid-cols-1 gap-4 p-4">
                        {Object.entries(SUGGESTED_RUMORS).map(
                          ([group, items]) => (
                            <div
                              key={group}
                              className="border border-blue-400 rounded-md p-2"
                            >
                              <p className="text-xs uppercase tracking-wide text-muted-foreground text-black font-bold mb-1">
                                {group}
                              </p>
                    ...
                    <div className="space-y-2">
                      ...
                      <Select
                        value={localTarget}
                        onValueChange={setLocalTarget}
                      >
                        <SelectTrigger>
                          <SelectValue placeholder="Ринок" />
                        </SelectTrigger>
                        <SelectContent>
                      ...
                      <p className="text-xs text-muted-foreground">
                        Натисніть на чутку, щоб застосувати.
                      </p>
                    </div>
                  </div>
                </CollapsibleContent>
                ...
                <CollapsibleContent>
                  <div className="mt-3 grid grid-cols-1 md:grid-cols-4 gap-3">
                    ...
                    <div className="flex items-end">
                      <Button className="w-full" onClick={createRumorServer}>
                        Надіслати
                      </Button>
                    </div>
                  </div>
              ...
              <div className="ml-auto flex items-center gap-1 text-sm">
                ...
                <Button
                  size="sm"
                  variant={speed === "2x" ? "default" : "outline"}
                  onClick={() => setSpeed("2x")}
                >
                  2x
                </Button>
                <Button
                  size="sm"
                  variant={speed === "4x" ? "default" : "outline"}
                  onClick={() => setSpeed("4x")}
                >
                  4x
                </Button>
          ...
          <CardContent className="space-y-4">
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                <p
                  className={`text-xl font-bold ${pnl >= 0 ? "text-emerald-600" : "text-red-600"}`}
                >
                  {formatCurrency(pnl)}
                </p>
              </div>
